...
const CircuitBackground = ({ 
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      const isLowPower = Boolean(connection?.saveData) || (deviceMemory > 0 && deviceMemory <= 4);
      const useSimpleStroke = prefersReducedMotion || isLowPower;
      ...
      const densityFactor = isMobile ? 20000 : 20000;
      const area = canvas.width * canvas.height;
      let nodeCount = Math.floor(area / densityFactor);
      const minNodes = isMobile ? 40 : 60;
      const maxNodes = isMobile ? 80 : 160;
      nodeCount = Math.max(minNodes, Math.min(maxNodes, nodeCount));
      if (isLowPower) {
        nodeCount = Math.max(minNodes, Math.floor(nodeCount * 0.6));
      }
      ...
      const CONNECTION_RADIUS = isMobile ? 400 : 540;
      const MAX_CONNECTIONS = isLowPower ? 2 : 4;
      ...
      const connectionInterval = window.setInterval(regenerateConnections, isLowPower ? 20000 : 12000);

      let animationFrame = 0;
      let time = 0;
      const timeStep = isMobile ? 0.025 : 0.05;
      const targetFps = prefersReducedMotion ? 0 : isLowPower ? 20 : isMobile ? 24 : 30;
